...
def main():
    ...
    parser.add_argument('--lambda-c', default=10, type=float,
                        help='coefficient of unlabeled loss')
    ...
    parser.add_argument('--expose', default=1, help="control level of confucius expose")
...
def train_one_epoch(args, labeled_trainloader, unlabeled_trainloader,
    ...
    for batch_idx, (data_x, data_u) in enumerate(train_loader):
        ...
        x_pred = x_pred.argmax(dim=1, keepdim=True)
        x_accu = x_pred.eq(targets_x.view_as(x_pred))
        true_confidence = (conf_x * x_accu).mean().item()
        false_confidence = (conf_x * ~x_accu).mean().item()

        x_conf_loss = F.binary_cross_entropy_with_logits(conf_logits_x, x_accu.float())
        ...
        if args.no_semi_confucius:
            ...
            if mask.sum() == 0:
                ...
                true_semi_confidence = (((u_s_accu * conf_u_s) * mask).sum() / mask.sum()).item()
                false_semi_confidence = (((~u_s_accu * conf_u_s) * mask).sum() / mask.sum()).item()
            u_s_conf_loss = (F.binary_cross_entropy_with_logits(
                conf_u_s, u_s_accu.float(), reduction="none") * mask).mean()

        conf_loss = (x_conf_loss + args.lambda_c * u_s_conf_loss) / ((args.lambda_c + 1) / 2)
